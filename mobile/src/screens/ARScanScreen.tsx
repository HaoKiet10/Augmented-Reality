...
interface Props {
  // ID lấy từ QR code / deep link (vd: myarapp://scan/<projectId>) hoặc nhập tay ở MVP
  projectId: string;
  onBack?: () => void;
}
...
const ARScanScreen: React.FC<Props> = ({ projectId, onBack }) => {
  ...
  const loadProject = async () => {
    ...
    try {
      ...
      registerProjectTrigger(data); // đăng ký tracking target 1 LẦN trước khi mount scene
      setProject(data);
      setLoadState('ready');
    } catch (err: any) {
