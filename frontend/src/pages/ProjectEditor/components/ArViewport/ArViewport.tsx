...
export function ArViewport({ aframeLoaded, activeAsset, position, rotation, scale }: ArViewportProps) {
    return (
        <main className="flex-1 bg-[#15171e] relative overflow-hidden flex flex-col justify-center items-center">
            {!aframeLoaded ? (
                ...
                <div className="absolute inset-0 w-full h-full">
                    <ArScene activeAsset={activeAsset} position={position} rotation={rotation} scale={scale} />
                    <ArStatusBadge activeAsset={activeAsset} />
                </div>
            )}
        </main>
    );
